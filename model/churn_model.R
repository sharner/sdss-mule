# This code has been adopted for this demonstration from
# https://tensorflow.rstudio.com/blog/keras-customer-churn.html
# It is chunked it into pieces we can invoked from a Mule ML Pipleine
...
churn_data_raw_tbl <- bake(rec_obj, newdata = churn_data_raw %>%
  select (-customerID)) %>% select(-Churn)
churn_data_raw_tbl <- bind_cols(churn_data_raw %>%
  select(customerID), churn_data_raw_tbl)
